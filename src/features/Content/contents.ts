...
export const contentDetail: ContentDetail[] = [
  ...
  },
  {
    ...
    techReason:
      "技術の選定理由としては、Ankiのアドオン開発で推薦されているPython, PyQtを軸に実装をしました。プロジェクトはオフライン利用可能かつオープンソースで公開したいという思いがあったため、条件に合ったCMU Pronouncing Dictionaryを使用しました。",
  ...
  },
  {
    ...
    background:
      "私は未知の単語に遭遇した際、いちいちネット検索することにストレスを感じていました。フラッシュカードへの単語登録と意味の検索を同時に行えれば学習効率が上がると考え、この開発に着手しました。",
    techReason:
      "使用技術はPythonとPyQtで、シンプルなUIを提供しています。サードパーティライブラリとしては、DeepLライブラリ、google-api-python-clientライブラリ、dateutilを使用しています。Ankiでは、サードパーティのライブラリのサポートがないです。そのため最低限のライブラリを使い、ライブラリと共に配布しています。",
    techStack: [python, qt, css],
  ...
  },
  {
    ...
    techReason:
      "モダンなWeb開発のためNext.js, React, TypeScriptを採用しました。ログインやデータベースをまとめて管理するためにFirebseを採用しました。状態管理にはZustandを使っています。ReduxなどではなくZustandを使用することにより、シンプルで設計で状態管理を可能としています。",
    techStack: [react, next, typescript, firebase],
